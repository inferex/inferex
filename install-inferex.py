"""
This script will install Inferex and its dependencies.

It does, in order:

  - Creates a virtual environment using venv (or virtualenv zipapp) in the correct OS data dir which will be
      - `%APPDATA%\\.inferex` on Windows
      -  ~/Library/Application Support/.inferex on MacOS
      - `${XDG_DATA_HOME}/.inferex` (or `~/.local/share/.inferex` if it's not set) on UNIX systems
      - In `${INFEREX_HOME}` if it's set.
  - Installs the latest or given version of Inferex inside this virtual environment.
  - Installs a `inferex` script in the Python user directory (or `${INFEREX_HOME/bin}` if `INFEREX_HOME` is set).
  - On failure, the error log is written to inferex-installer-error-*.log and any previously existing environment
    is restored.
"""
...
def data_dir(version: Optional[str] = None) -> Path:
    if os.getenv("INFEREX_HOME"):
        return Path(os.getenv("INFEREX_HOME")).expanduser()

    if WINDOWS:
        const = "CSIDL_APPDATA"
        path = os.path.normpath(_get_win_folder(const))
        path = os.path.join(path, ".inferex")
    elif MACOS:
        path = os.path.expanduser("~/Library/Application Support/.inferex")
    else:
...
def bin_dir(version: Optional[str] = None) -> Path:
    if os.getenv("INFEREX_HOME"):
        return Path(os.getenv("INFEREX_HOME"), "bin").expanduser()
...
PRE_MESSAGE = """# Welcome to {inferex}!

This will download and install the latest version of {inferex},
a dependency and package manager for Python.

It will add the `inferex` command to {inferex}'s bin directory, located at:
...
POST_MESSAGE = """{inferex} ({version}) is installed now. Great!
...
POST_MESSAGE_NOT_IN_PATH = """{inferex} ({version}) is installed now. Great!

To get started you need {inferex}'s bin directory ({inferex_home_bin}) in your `PATH`
environment variable.
{configure_message}
Alternatively, you can call {inferex} explicitly with `{inferex_executable}`.
...
Add `export PATH="{inferex_home_bin}:$PATH"` to your shell configuration file.
"""

POST_MESSAGE_CONFIGURE_FISH = """
You can execute `set -U fish_user_paths {inferex_home_bin} $fish_user_paths`
"""

POST_MESSAGE_CONFIGURE_WINDOWS = """"""


class InferexInstallationError(RuntimeError):
    def __init__(self, return_code: int = 0, log: Optional[str] = None):
        super(InferexInstallationError, self).__init__()
...
class VirtualEnvironment:
    ...
    def make(cls, target: Path) -> "VirtualEnvironment":
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="inferex-installer") as temp_dir:
                virtualenv_pyz = Path(temp_dir) / "virtualenv.pyz"
                request = Request(
                    virtualenv_bootstrap_url, headers={"User-Agent": "Python Inferex"}
                )
        ...
        # We add a special file so that Inferex can detect
        # its own virtual environment
        target.joinpath("inferex_env").touch()
    ...
    def run(*args, **kwargs) -> subprocess.CompletedProcess:
        ...
        if completed_process.returncode != 0:
            raise InferexInstallationError(
...
class Installer:
    METADATA_URL = "https://pypi.org/pypi/inferex/json"
    ...
    def install(self, version):
        """
        Installs Inferex in $INFEREX_HOME.
        """
        self._write(
            "Installing {} ({})".format(
                colorize("info", "Inferex"), colorize("info", version)
            )
        )

        with self.make_env(version) as env:
            self.install_inferex(version, env)
    ...
    def uninstall(self) -> int:
        if not self._data_dir.exists():
            self._write(
                "{} is not currently installed.".format(colorize("info", "Inferex"))
            )
        ...
        if version:
            self._write(
                "Removing {} ({})".format(
                    colorize("info", "Inferex"), colorize("b", version)
                )
            )
        else:
            self._write("Removing {}".format(colorize("info", "Inferex")))

        shutil.rmtree(str(self._data_dir))
        for script in ["inferex", "inferex.bat"]:
    ...
    def make_bin(self, version: str, env: VirtualEnvironment) -> None:
        ...
        script = "inferex.exe" if WINDOWS else "inferex"
    ...
    def install_inferex(self, version: str, env: VirtualEnvironment) -> None:
        self._install_comment(version, "Installing Inferex")
        ...
        if self._git:
            ...
            specification = f"inferex=={version}"

        env.pip("install", specification)

    def display_pre_message(self) -> None:
        kwargs = {
            "inferex": colorize("info", "Inferex"),
            "inferex_home_bin": colorize("comment", self._bin_dir),
        }
    ...
    def display_post_message_windows(self, version: str) -> None:
        ...
        self._write(
            message.format(
                ...
                inferex_home_bin=colorize("comment", self._bin_dir),
                inferex_executable=colorize("b", self._bin_dir.joinpath("inferex")),
                configure_message=POST_MESSAGE_CONFIGURE_WINDOWS.format(
                    inferex_home_bin=colorize("comment", self._bin_dir)
                ),
                test_command=colorize("b", "inferex --version"),
            )
        )
    ...
    def get_version(self):
        ...
        self._write(colorize("info", "Retrieving Inferex metadata"))
    ...
    def _get(self, url):
        request = Request(url, headers={"User-Agent": "Python Inferex"})
...
def main():
    parser = argparse.ArgumentParser(
        description="Installs the latest (or given) version of inferex"
    )
    parser.add_argument(
        "-p",
        "--preview",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "fetching the latest version of Inferex available online."
        ),
    )
    parser.add_argument(
        "--git",
    ...
    installer = Installer(
        version=args.version or os.getenv("INFEREX_VERSION"),
        preview=args.preview or string_to_bool(os.getenv("INFEREX_PREVIEW", "0")),
        force=args.force,
        accept_all=args.accept_all
        or string_to_bool(os.getenv("INFEREX_ACCEPT", "0"))
        or not is_interactive(),
        path=args.path,
        git=args.git,
    )

    if args.uninstall or string_to_bool(os.getenv("INFEREX_UNINSTALL", "0")):
        return installer.uninstall()

    try:
        return installer.run()
    except InferexInstallationError as e:
        installer._write(colorize("error", "Inferex installation failed."))
        ...
        if e.log is not None:
            ...
            _, path = tempfile.mkstemp(
                suffix=".log",
                prefix="inferex-installer-error-",
                dir=str(Path.cwd()),
                text=True,
            )
